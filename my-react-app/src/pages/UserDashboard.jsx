 ...
 * 
 * relative path: /src/pages/UserDashboard.jsx
 * 
 * This is the heart of the UniBook experience for regular clients. 
 * It is designed to be a high-performance "Control Room" where users 
 * can see their immediate schedule and discover new services.
 * 
 * Logic & Orchestration:
 * - Smart Sorting: We fetch all user bookings, filter out the cancelled ones, 
 *   and sort them chronologically to determine the "Next Big Thing".
 * - Hero Focus: The top card dynamically highlights the very next 
 *   confirmed appointment to ensure it's the first thing the user sees.
 * - AI Concierge: Integrates the AI ChatBot for real-time, 
...
const UserDashboard = () => {
   ...
   useEffect(() => {
      ...
      // Load user identity from local session profile
      setUser(getProfile());

      /**
       * fetchBookings
       * Pulls the master list of appointments for this specific user 
       * and performs the chronological sorting logic.
       */
      const fetchBookings = async () => {
         try {
            ...
            // Step 1: Remove clutter (cancelled bookings)
            const active = data.filter(b => b.status !== 'cancelled');
            
            // Step 2: Precise Chronological Sorting
            active.sort((a, b) => {
               ...
               // If dates match, compare time strings
   ...
   // The "Single Source of Truth" for the user's nearest appointment
            ...
            <div className="max-w-7xl mx-auto w-full pt-16 relative z-10">
               ...
               <div className="grid grid-cols-12 gap-8">
                  ...
                  {/* 
                      ...
                      Occupies the most visual real-estate as it's the 
                      highest priority item for a user.
                  */}
                  <div className={`col-span-12 lg:col-span-8 group relative rounded-2xl p-8 overflow-hidden transition-all duration-200 border glass-card`}>
                     ...
                     <div className="relative z-10 py-4">
                        ...
                        {nextAppointment ? (
                           ...
                           // Empty State: Encourages the user to make their first booking
                  ...
                  {/* 
                      ...
                      Secondary tools for navigation.
                  */}
               ...
               {/* Bottom Navigation: Quick Category Jump */}

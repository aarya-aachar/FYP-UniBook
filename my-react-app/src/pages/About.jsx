  ...
  return (
    <div className="flex flex-col min-h-screen transition-all duration-500 font-inter"
      ...
      <main className="flex-1 overflow-y-auto px-6 md:px-10 py-16 relative flex flex-col items-center transition-all duration-500">
        <div className={`absolute top-[10%] left-[10%] w-96 h-96 blur-[120px] rounded-full pointer-events-none transition-all duration-500 ${isDark ? 'bg-blue-600/10' : 'bg-blue-400/5'}`} />
        ...
        <div className="max-w-4xl mx-auto w-full pt-4 relative z-10">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {[
                ...
                <div key={item.title} className={`p-6 rounded-2xl border transition-all ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                   <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-colors ${isDark ? 'bg-slate-800 text-blue-400' : 'bg-white text-blue-600 border border-slate-100 shadow-sm'}`}>{item.icon}</div>
          ...
          <div className={`rounded-3xl p-10 md:p-12 border backdrop-blur-xl text-center transition-all ${isDark ? 'bg-gradient-to-br from-blue-600/10 to-indigo-600/5 border-blue-500/20' : 'bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-100'}`}>
             ...
             <Link to="/services" className={`inline-flex items-center gap-2 px-8 py-3 rounded-xl font-semibold text-sm transition-all shadow-sm
                ${isDark ? 'bg-blue-600 text-white hover:bg-blue-500 hover:shadow' : 'bg-blue-600 text-white hover:bg-blue-700 hover:shadow-md'}`}>
